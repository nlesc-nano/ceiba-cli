...
__all__ = ["list_container", "save_large_objects"]
...
def list_container(container: str) -> Iterable[str]:
    """List the container entry."""
    return execute_swift_action("list", container)


def save_large_objects(info: Options, prop_data: Dict[str, Any]) -> Dict[str,Any]:
    ...
    check_action(execute_swift_action("post", container))
    reply = check_action(execute_swift_action("upload", container, objects=files))
